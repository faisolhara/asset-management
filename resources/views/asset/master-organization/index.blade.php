        ...
        <form id="form-delete" role="form" method="post" action="{{ URL($url . '/delete') }}">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="p_organization_name" >Are you sure to delete?</h4> 
                </div>
                    ...
                    <table id="myTable" class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $model->remarks }}</td>
                                <td class="text-right">
                                    <a data-id="{{ $model->organization_id }}" data-model="{{ json_encode($model) }}" data-toggle="modal" data-target="#info-modal" class="btn btn-info btn-outline btn-circle btn-xs m-r-5 md-trigger info-action" data-original-title="Information" data-modal="modal-info" >
                                        <i class="fa fa-info"></i>
                                    </a>
                                    <a type="button" href="{{ url($url.'/edit').'/'.$model->organization_id }}" class="btn btn-warning btn-outline btn-circle btn-xs m-r-5">
                                        <i class="ti-pencil-alt"></i>
                                    </a>
                                    <a data-id="{{ $model->organization_id }}" data-label="{{ $model->organization_name }}" data-toggle="modal" data-target="#delete-modal" class="btn btn-danger btn-outline btn-circle btn-xs m-r-5 md-trigger delete-action" data-original-title="Delete" data-modal="modal-delete" >
...
<script type="text/javascript">
    ...
    $('.info-action').on('click', function() {
        ...
        $("#i_organization_name").html(': ' + $model.organization_name);
    ...
    $('.delete-action').on('click', function() {
        $("#p_organization_id").val($(this).data('id'));
        $("#p_organization_name").html('Are you sure want to delete ' + $(this).data('label') + '?');
    });
